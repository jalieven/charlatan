...
| Skipped round (§3.10) | 0 for everyone — the round is void, correct-vote bonuses included |
...
1. **Setup flow.** The host enters player names (in any order — every round shuffles the
   pass order, §3.4), adjusts Charlatan count, clues-per-player, Whisper cards per player,
   the three handicap switches (Blind bonuses, Deaf & stone, Reshuffle clue order — §2.2),
   and language, and taps **Start**. Validation:
