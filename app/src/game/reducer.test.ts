...
describe('the self-handicap ladder (§2.3, §3.4, §3.9)', () => {
  ...
  function roundWith(players: RoundPlayer[], outcome: RoundOutcome): RoundState {
    return {
      ...
      pair: {
        real: 'koffie',
        realDef: 'warme drank van gebrande bonen',
        decoy: 'thee',
        decoyDef: 'warme drank van gedroogde blaadjes',
        distractors: [],
        distractorDefs: [],
      },
      players,
  ...
  it('whisper targeting stays in reveal positions when the clue order diverges', () => {
    ...
    s = reducer(s, { type: 'PEEK' })
    const before = s
    s = reducer(s, { type: 'BURN_WHISPER', targetSeat: 1, fakeWord: 'espresso', fakeWordDef: 'x', swapped: false })
    expect(s).toBe(before)
    s = reducer(before, { type: 'BURN_WHISPER', targetSeat: 2, fakeWord: 'espresso', fakeWordDef: 'x', swapped: false })
    expect(s.round!.whisper!.target).toBe('Tom')
  })
